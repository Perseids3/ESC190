...
/////// Lab 8
bool is_avl_node(avl_node_t *root)
{
    if(!root)
        return true;
    int h1 = (int)HEIGHT(root->left);
    int h2 = (int)HEIGHT(root->right);
    if((h1-h2) > 1 || (h1-h2) < -1){
        return false;
    }
    
    if(!is_avl_node(root->left))
        return false;
    
    if(!is_avl_node(root->right))
        return false;
                ...
                int (*cmp)(bag_elem_t, bag_elem_t))
{
    ...
    if (! *root) {
        ...
        if ((inserted = avl_insert_norot(&(*root)->left, elem, cmp))) {
              avl_update_height(*root);
        }
    } else if ((*cmp)(elem, (*root)->elem) > 0) {
        if ((inserted = avl_insert_norot(&(*root)->right, elem, cmp))) {
                avl_update_height(*root);
        }
    } else { /* ((*cmp)(elem, (*root)->elem) == 0) */
        inserted = avl_insert_norot(&(*root)->right, elem, cmp);
        /* No rebalancing necessary, but update height. */
        if (inserted)  avl_update_height(*root);
    }
...
bool bag_insert_norot(bag_t *b, bag_elem_t e)
{
    return avl_insert_norot(&(b->root), e, b->cmp);
}
